...
get(site_name, api_base_url=None, use_pure_rest_api=False):

    db_stores = frappe.db.sql('select name, oc_store_id from `tabOpencart Store` where oc_site = \'%s\'' % site_name, as_dict=1)
    for db_store in db_stores:
        doc_store = frappe.get_doc('Opencart Store', db_store.get('name'))
        for oc_order in oc_api.get(site_name).get_orders_by_customer(db_store.get('oc_store_id')):


    db_customers = frappe.db.sql('select oc_customer_id from `tabItem` where oc_site = \'%s\'' % site_name, as_dict=1)
