    ...
    if sales_order.is_oc_sales_order(doc):
        if sales_order.is_oc_lustcobox_order_doc(doc):
            ...
            if not recurring_profile:
                ...
                recurring_profile = frappe.db.get_value("Recurring Profile", {"sales_order": doc.name}, "name")
            if is_converge and not doc.oc_cc_token_id:
                frappe.throw("You cannot submit Sales Order {} due to CC Token Id is not set".format(doc.name))
            if (is_stripe or is_paypal) and not doc.oc_cheque_no and not doc.oc_cheque_date:
...
def add_lustcobox_order_part(doc_sales_order, oc_order=None):
    ...
    recurring_profile_doc.update({
        ...
        "skip_month": cint(lustcobox.get("skip_month")),
...
def add_lustcobox_order_part_to_profile(recurring_profile, so):
    ...
    if not get_order_success:
        frappe.throw("Cannot get Sales Order with order id {} from Opencart Site {}".format(doc_sales_order.oc_order_id, doc_sales_order.oc_site))
...
def sync_lustcobox_order_part_from_oc(so):
